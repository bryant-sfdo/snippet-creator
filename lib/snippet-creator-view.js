...
export default class SnippetCreatorView {
  ...
  render () {
    return (
      ...
      <h2 class='block'>Snippet Creator</h2>
        ...
        <div>
        <div className='snippet-creator-editor' ref="sourceEditor">
          <TextEditor
            mini={true}
            ref='source'
            softWrapped={true}
        ...
        <div className='block'>
          <a href="#" onclick={this.viewSourceTypes.bind(this)} className="btn">View Source Types</a>
        </div>
  ...
  getSnippetFile(){
    return path.join(atom.configDirPath, "snippets.json");//path.join(atom.packages.getActivePackage('snippet-creator').path, 'snippets.json');
  }
  openFile(){
  ...
  //If required text isn't filled out, return focus and set error boarder
  setErrorAndFocus(){
